...
def _delegated(addr: address) -> bool:
    return self.voters[addr] != ZERO_ADDRESS
...
def _directlyVoted(addr: address) -> bool:
    return self.voters[addr] and (self.voters[addr] == ZERO_ADDRESS)
...
def giveRightToVote(voter: address):
    ...
    assert not self.voters[voter]
    # Throws if the voter's voting weight isn't 0.
    assert self.voters[voter] == 0
    self.voters[voter] = 1
    self.voterCount += 1
...
def _forwardWeight(delegate_with_weight_to_forward: address):
    ...
    assert self.voters[delegate_with_weight_to_forward] > 0

    target: address = self.voters[delegate_with_weight_to_forward]
    for i in range(4):
        if self._delegated(target):
            target = self.voters[target]
    ...
    weight_to_forward: int128 = self.voters[delegate_with_weight_to_forward]
    self.voters[delegate_with_weight_to_forward] = 0
    self.voters[target] += weight_to_forward

    if self._directlyVoted(target):
        self.proposals[self.voters[target]] += weight_to_forward
        self.voters[target] = 0
...
def delegate(to: address):
    ...
    assert not self.voters[msg.sender]
...
def vote(proposal: int128):
    ...
    self.voters[msg.sender] = proposal
    self.voters[msg.sender] = True

    # transfer msg.sender's weight to proposal
    self.proposals[proposal] += self.voters[msg.sender]
    self.voters[msg.sender] = 0
...
def _winningProposal() -> int128:
    ...
    for i in range(2):
        if self.proposals[i] > winning_vote_count:
            winning_vote_count = self.proposals[i]
            winning_proposal = i
...
def winnerName() -> bytes32:
    return self.proposals[self._winningProposal()]
